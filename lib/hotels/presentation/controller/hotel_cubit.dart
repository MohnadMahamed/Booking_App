...
class HotelCubit extends Cubit<HotelState> {
  HotelCubit(this.registerUseCase,this.loginUseCase,this.updateUserInfoUseCase,this.updateBookingStatusUseCase,this.searchHotelsUseCase,this.getFacilitiesUseCase,this.getBookingsUseCase,this.getAllHotelsUseCase,this.createBookingUseCase) : super(HotelInitial());
  ...
  Future<Either<Failure, UserDataModel>> register(RegisterRequestModel registerRequest)async{
...
return result;
  ...
  Future<Either<Failure, StatusModel>> createBookings(int hotelId ,int userId)async{
  emit(CreateBookingLoadingState());

  final result=await createBookingUseCase.call(userId, hotelId);
  result.fold((l) { ServerFailure(l.message);  emit(HotelErrorState());
  }, (r){createBookingResult=r;
...
return result;
  ...
  Future<Either<Failure, StatusModel>> updateBookings(String type ,int bookingId)async{
  emit(UpdateBookingLoadingState());

  final result=await updateBookingStatusUseCase.call(bookingId, type);
  result.fold((l){ ServerFailure(l.message);  emit(HotelErrorState());
  }, (r) {updateBookingResult=r;
  ...
  final result=await searchHotelsUseCase.call(name:name ,address: address,minPrice: minPrice,maxPrice: maxPrice,count: count,page: page,longitude: longitude,latitude: latitude,distance: distance);
  result.fold((l) { ServerFailure(l.message);  emit(HotelErrorState());
  }, (r) {
    searchHotelList = r;
